...
export function CreateEnquiryDialog({ open, onOpenChange, onCreated }: CreateEnquiryDialogProps) {
  ...
  const resetForm = () => {
    setCompanyName(''); setContactName(''); setContactPhone('');
    setContactPhoneAlt(''); setContactEmail(''); setCity('');
    setSource(EnquirySource.CALL_DIRECT); setTenancyType('');
    setWhatsappEnabled(false); setErrors({});
  };
