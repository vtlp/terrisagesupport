...
const channelLabel: Record<CampaignChannel, string> = {
  META: 'Meta', GOOGLE: 'Google', LINKEDIN: 'LinkedIn', X: 'X',
...
export default function MktCampaignsTab() {
  ...
  const openCreate = () => {
    setEditing(null);
    setForm({
      campaign_name: '', channel: CampaignChannel.META, objective: CampaignObjective.LEAD_GEN,
      segment_target: SegmentTarget.BOTH, geo_cities: [], start_date: '', end_date: '',
      status: CampaignStatus.DRAFT, owner_user_id: 'U001', planned_budget: 0, notes: '',
    });
    setDrawerOpen(true);
  };
  ...
  const handleSave = () => {
    ...
    if (editing) {
      ...
      const newCampaign: Campaign = {
        campaign_id: `CMP${String(campaigns.length + 1).padStart(3, '0')}`,
        campaign_name: form.campaign_name || '',
        channel: form.channel || CampaignChannel.META,
        objective: form.objective || CampaignObjective.LEAD_GEN,
        segment_target: form.segment_target || SegmentTarget.BOTH,
        geo_cities: form.geo_cities || [],
        start_date: form.start_date || '',
        end_date: form.end_date || '',
        status: form.status || CampaignStatus.DRAFT,
        owner_user_id: form.owner_user_id || 'U001',
        planned_budget: form.planned_budget || 0,
        notes: form.notes || '',
        linked_assets: [],
          ...
          <div className="overflow-x-auto">
            <Table>
              ...
              <TableBody>
                ...
                {filtered.length === 0 && (
                  <TableRow><TableCell colSpan={12} className="text-center py-12 text-muted-foreground">No campaigns found</TableCell></TableRow>
                )}
              </TableBody>
            </Table>
          </div>
      ...
      <Sheet open={drawerOpen} onOpenChange={setDrawerOpen}>
        <SheetContent className="w-full sm:max-w-lg overflow-y-auto bg-card">
          ...
          <div className="space-y-4 mt-6">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                ...
                <Select value={form.channel || ''} onValueChange={v => setForm(f => ({ ...f, channel: v as CampaignChannel }))}>
                  ...
                  <SelectContent className="bg-card">
                    {Object.values(CampaignChannel).map(ch => <SelectItem key={ch} value={ch}>{channelLabel[ch]}</SelectItem>)}
                  </SelectContent>
                </Select>
              </div>
              <div>
                ...
                <Select value={form.objective || ''} onValueChange={v => setForm(f => ({ ...f, objective: v as CampaignObjective }))}>
                  <SelectTrigger><SelectValue /></SelectTrigger>
                  <SelectContent className="bg-card">
                    {Object.values(CampaignObjective).map(o => <SelectItem key={o} value={o}>{objectiveLabel[o]}</SelectItem>)}
                  </SelectContent>
                </Select>
              </div>
            </div>
            <div className="grid grid-cols-2 gap-3">
              <div>
                ...
                <Select value={form.segment_target || ''} onValueChange={v => setForm(f => ({ ...f, segment_target: v as SegmentTarget }))}>
                  <SelectTrigger><SelectValue /></SelectTrigger>
                  <SelectContent className="bg-card">
                    {Object.values(SegmentTarget).map(s => <SelectItem key={s} value={s}>{s}</SelectItem>)}
                  </SelectContent>
                </Select>
              </div>
              <div>
            ...
            <div>
              <Label className="text-xs">Planned Budget (₹)</Label>
              <Input type="number" value={form.planned_budget || ''} onChange={e => setForm(f => ({ ...f, planned_budget: Number(e.target.value) }))} />
            </div>
            <div>
              <Label className="text-xs">Cities (comma-separated)</Label>
              <Input value={form.geo_cities?.join(', ') || ''} onChange={e => setForm(f => ({ ...f, geo_cities: e.target.value.split(',').map(s => s.trim()).filter(Boolean) }))} />
            </div>
            <div>
            ...
            {editing && (editing.utm_campaign || editing.utm_source) && (
              <div className="bg-muted/50 rounded-lg p-3">
                <p className="text-xs font-medium text-muted-foreground mb-1">UTM Bundle (read-only)</p>
