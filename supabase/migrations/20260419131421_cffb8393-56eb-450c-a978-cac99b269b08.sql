-- Sequences for account codes and invoice numbers
CREATE SEQUENCE IF NOT EXISTS public.account_code_seq START 1;
CREATE SEQUENCE IF NOT EXISTS public.invoice_no_seq START 1;

-- Trigger: auto-assign ACC-000001 style codes to accounts
CREATE OR REPLACE FUNCTION public.accounts_set_code()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  IF NEW.account_code IS NULL OR NEW.account_code = '' THEN
    NEW.account_code := 'ACC-' || LPAD(nextval('public.account_code_seq')::TEXT, 6, '0');
  END IF;
  RETURN NEW;
END $$;

DROP TRIGGER IF EXISTS trg_accounts_set_code ON public.accounts;
CREATE TRIGGER trg_accounts_set_code
BEFORE INSERT ON public.accounts
FOR EACH ROW EXECUTE FUNCTION public.accounts_set_code();

-- Trigger: auto-assign INV-000001 style codes to invoices
CREATE OR REPLACE FUNCTION public.invoices_set_no()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
...
DROP TRIGGER IF EXISTS trg_invoices_set_no ON public.account_invoices;
CREATE TRIGGER trg_invoices_set_no
BEFORE INSERT ON public.account_invoices
FOR EACH ROW EXECUTE FUNCTION public.invoices_set_no();

-- Backfill existing rows that lack codes, in stable order
DO $$
DECLARE r RECORD; v_max INT;
BEGIN
  FOR r IN SELECT id FROM public.accounts WHERE account_code IS NULL OR account_code = '' ORDER BY created_at ASC, id ASC LOOP
    UPDATE public.accounts SET account_code = 'ACC-' || LPAD(nextval('public.account_code_seq')::TEXT, 6, '0') WHERE id = r.id;
  END LOOP;

  FOR r IN SELECT id FROM public.account_invoices WHERE invoice_no IS NULL OR invoice_no = '' ORDER BY created_at ASC, id ASC LOOP
    UPDATE public.account_invoices SET invoice_no = 'INV-' || LPAD(nextval('public.invoice_no_seq')::TEXT, 6, '0') WHERE id = r.id;
  END LOOP;
END $$;

-- Update convert_enquiry_to_account: invoice seat_count must reflect SEATS PURCHASED, not active members
...
BEGIN
  ...
  -- Seats purchased drives both billing and invoice line — independent of active members.
