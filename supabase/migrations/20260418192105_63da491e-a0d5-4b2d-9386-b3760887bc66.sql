...
BEGIN
  ...
  -- Copy onboarding free-text notes (from form) into account notes if present
  v_onboarding_notes := NULLIF(TRIM(COALESCE(v_payload->>'notes', '')), '');
  IF v_onboarding_notes IS NOT NULL THEN
    INSERT INTO public.account_notes (account_id, author_id, note_text)
      VALUES (v_account_id, auth.uid(), 'From onboarding form: ' || v_onboarding_notes);
  END IF;
