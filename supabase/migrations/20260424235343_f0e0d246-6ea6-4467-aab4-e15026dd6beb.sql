...
BEGIN
  ...
  v_owner_name  := COALESCE(v_payload->>'owner_name',  v_enquiry.full_name);
  v_owner_phone := COALESCE(v_payload->>'owner_phone', v_enquiry.phone);
  v_owner_email := COALESCE(v_payload->>'owner_email', v_enquiry.email);
  ...
  IF v_owner_name IS NOT NULL AND TRIM(v_owner_name) <> '' THEN
    INSERT INTO public.account_seats (
      account_id, full_name, email, phone, role, is_active, is_superuser
    ) VALUES (
      v_account_id, v_owner_name, v_owner_email, v_owner_phone, 'Super User', true, true
    );
  ...
  v_sub_start := NULLIF(v_payment #>> '{subscription,start_at}', '')::TIMESTAMPTZ;
  v_sub_end   := NULLIF(v_payment #>> '{subscription,end_at}',   '')::TIMESTAMPTZ;
  ...
  v_next_renewal := CASE
    ...
    WHEN v_sub_end IS NOT NULL THEN v_sub_end
  ...
  ON CONFLICT (account_id) DO UPDATE
    SET plan_name = EXCLUDED.plan_name,
        ...
        subscription_started_at = COALESCE(public.account_billing_settings.subscription_started_at, EXCLUDED.subscription_started_at),
        current_period_start = COALESCE(public.account_billing_settings.current_period_start, EXCLUDED.current_period_start),
        current_period_end = COALESCE(public.account_billing_settings.current_period_end, EXCLUDED.current_period_end),
