-- 1) Update convert_enquiry_to_account to hydrate billing settings + paid invoice from enquiry payment payload
...
BEGIN
  ...
  -- Pull payment + breakdown from the enquiry payload (set by Razorpay link generator).
  ...
  v_seats_purchased := GREATEST(v_seats_required, COALESCE(v_invoice_seats, 0));

  INSERT INTO public.account_billing_settings (
    account_id, plan_name, billing_cycle, base_fee, seat_rate, gst_pct, seats_purchased, status
  ) VALUES (
    v_account_id, v_plan_name, v_cycle::billing_cycle, v_base_fee, v_seat_rate, v_gst_pct, v_seats_purchased, 'ACTIVE'
  )
  ...
  -- If the payment link was paid, create a matching PAID invoice on the new account.
...
-- 2) Backfill: for every already-converted account whose source enquiry has payment data,
-- hydrate billing settings + add a PAID invoice (skip if a PAID invoice already exists for that link).
DO $$
DECLARE
  rec RECORD;
  v_breakdown JSONB;
...
BEGIN
  FOR rec IN
    SELECT a.id AS account_id, e.payload->'payment' AS payment
    FROM public.accounts a
    JOIN public.enquiries e ON e.id = a.source_enquiry_id
    WHERE e.payload ? 'payment'
  LOOP
    v_breakdown := COALESCE(rec.payment->'breakdown', '{}'::jsonb);
    v_plan_name := COALESCE(NULLIF(v_breakdown->>'plan_name', ''), 'Standard');
    v_cycle     := COALESCE(NULLIF(v_breakdown->>'billing_cycle', ''), 'ANNUAL');
    v_base_fee  := COALESCE(NULLIF(v_breakdown->>'base_fee', '')::NUMERIC, 33000);
    v_seat_rate := COALESCE(NULLIF(v_breakdown->>'per_seat_rate', '')::NUMERIC, 7000);
    v_gst_pct   := COALESCE(NULLIF(v_breakdown->>'gst_pct', '')::NUMERIC, 18);
    v_invoice_seats := COALESCE(NULLIF(v_breakdown->>'seats', '')::INT, 0);

    SELECT COALESCE(seats_purchased, 0) INTO v_existing_seats
    FROM public.account_billing_settings WHERE account_id = rec.account_id;
    v_seats_purchased := GREATEST(COALESCE(v_existing_seats, 0), v_invoice_seats);

    INSERT INTO public.account_billing_settings (
      account_id, plan_name, billing_cycle, base_fee, seat_rate, gst_pct, seats_purchased, status
    ) VALUES (
      rec.account_id, v_plan_name, v_cycle::billing_cycle, v_base_fee, v_seat_rate, v_gst_pct, v_seats_purchased, 'ACTIVE'
    )
    ON CONFLICT (account_id) DO UPDATE
      SET plan_name = EXCLUDED.plan_name,
          billing_cycle = EXCLUDED.billing_cycle,
    ...
    IF (rec.payment->>'status') = 'PAID' THEN
      v_short_url := rec.payment->>'short_url';
      v_link_id   := rec.payment->>'link_id';
      v_subtotal   := COALESCE(NULLIF(v_breakdown->>'subtotal', '')::NUMERIC,    v_base_fee + v_seat_rate * GREATEST(v_invoice_seats - 3, 0));
      v_gst_amount := COALESCE(NULLIF(v_breakdown->>'gst_amount', '')::NUMERIC,  v_subtotal * v_gst_pct / 100);
      v_total      := COALESCE(NULLIF(v_breakdown->>'total', '')::NUMERIC,       v_subtotal + v_gst_amount);
      v_paid_at    := COALESCE(NULLIF(rec.payment->>'paid_at', '')::TIMESTAMPTZ, now());

      IF NOT EXISTS (
        SELECT 1 FROM public.account_invoices
        WHERE account_id = rec.account_id
          AND status = 'PAID'
          AND notes IS NOT NULL
          AND (notes LIKE '%' || COALESCE(v_short_url, '___none___') || '%'
               OR notes LIKE '%' || COALESCE(v_link_id, '___none___') || '%')
      ) THEN
        INSERT INTO public.account_invoices (
          account_id, plan_name, seat_count, seat_rate, base_fee,
          subtotal, gst_pct, gst_amount, total, status,
          ...
          rec.account_id, v_plan_name, v_invoice_seats, v_seat_rate, v_base_fee,
          v_subtotal, v_gst_pct, v_gst_amount, v_total, 'PAID',
          COALESCE(NULLIF(rec.payment->>'created_at', '')::TIMESTAMPTZ, v_paid_at), v_paid_at,
          'Razorpay payment link · ' || COALESCE(v_short_url, v_link_id, '—')
        );
      END IF;
    END IF;
  END LOOP;
