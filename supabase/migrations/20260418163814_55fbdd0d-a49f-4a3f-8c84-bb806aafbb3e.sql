...
BEGIN
  ...
  -- Determine seats_required from onboarding payload (team.seats_required), fallback to actual member count
  v_seats_required := GREATEST(
    COALESCE(NULLIF((v_payload #>> '{team,seats_required}'), '')::INT, 0),
    v_member_count
  );

  -- Seed billing settings with seats_purchased (allocated) from onboarding form
