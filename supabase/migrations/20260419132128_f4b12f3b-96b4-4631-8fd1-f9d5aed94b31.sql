...
BEGIN
  ...
  -- Compute next renewal from conversion date + billing cycle
  v_next_renewal := CASE v_cycle
  ...
  INSERT INTO public.account_billing_settings (
    account_id, plan_name, billing_cycle, base_fee, seat_rate, gst_pct, seats_purchased, status, next_renewal_at
  ) VALUES (
    v_account_id, v_plan_name, v_cycle::billing_cycle, v_base_fee, v_seat_rate, v_gst_pct, v_seats_purchased, 'ACTIVE', v_next_renewal
  )
...
-- Backfill: set next_renewal_at for existing accounts that don't have one,
-- using account.created_at + the billing cycle.
UPDATE public.account_billing_settings abs
SET next_renewal_at = a.created_at + CASE abs.billing_cycle
...
WHERE abs.account_id = a.id
  AND abs.next_renewal_at IS NULL;